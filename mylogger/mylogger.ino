...
//  strcpy(&csvName[BASE_NAME_SIZE + 3], "csv");
  sprintf(csvName, "%s.CSV", binName);
  Serial.print(csvName);
  Serial.println(F(" <- wird bearbeitet!"));
//##########
...
void logData(char MY_FILE_NAME[]) {
  uint32_t bgnBlock, endBlock;
...
//  #######
//  BASE_NAME_SIZE hinfällig da eigener NAME
//  #######
...
//  #######
//  MY_FILE_NAME als Funktionsgegebener Parameter
//  #######
...
// NAME = MY_FILE_NAME , SIZE = 512 * 256000
  Serial.println(F("Creating new file"));
  binFile.close();
  //                   "char MY_FILE_NAME[]"  1 Block * 256000
  if (!binFile.createContiguous(MY_FILE_NAME, 512 * FILE_BLOCK_COUNT)) {
    error("createContiguous failed");
  }

// bgnBlock = Erste Adresse des FILES 
// endBlock = Letzte Adresse des FILES
  if (!binFile.contiguousRange(&bgnBlock, &endBlock)) {
    error("contiguousRange failed");
  }
...
// LÖSCHEN ALLER DATEN
// START UND ENDBIT in Erase schreiben
// BSP: BLOCK 50 - BLOCK 200
// erase ( 50 , 200 )
// after => bgn=50 wird auf 200+1 geändert für nächste Löschung
  Serial.println(F("Erasing all data"));
  uint32_t bgnErase = bgnBlock;
  uint32_t endErase;
  while (bgnErase < endBlock) {
    endErase = bgnErase + ERASE_SIZE;
    // Falls endErase überläuft
...
// MEHRERE BLÖCKE
//           Wir starten mit bgnBlock      256000 Blöcke frei machen
  if (!sd.card()->writeStart(bgnBlock, FILE_BLOCK_COUNT)) {
    error("writeBegin failed");
  }
// MEHRERE BLÖCKE
// block[0] is der Pointer auf die Daten
  if (!sd.card()->writeData((uint8_t*)&block[0])) {
    error("Write metadata failed");
  }

// Den Blöcken das Datanformat einbrennen
// Initialize queues emptyHead
